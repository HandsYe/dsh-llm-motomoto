window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			title: "MotoMoto 中转站",
			description: "MotoMoto 模型路由的状态。模型位于模型选择器的 MotoMoto 分组；密钥通过凭据 MOTOMOTO_API_KEY 提供。",
			group: "模型分组",
			endpoint: "端点",
			credential: "凭据引用",
			models: "模型",
			...
			missing: "未找到 motomoto 路由。请在「设置 → 模型」的 llm-pi-ai 提供方中配置，或重装本插件以恢复默认路由。",
		};
		/** English dictionary checked against the Chinese key set. */
		const en = {
			title: "MotoMoto relay",
			description: "Status of the MotoMoto model route. The models sit in the MotoMoto group of the model picker; the key is supplied as the MOTOMOTO_API_KEY credential.",
			group: "Model group",
			endpoint: "Endpoint",
			...
			missing: "No motomoto route found. Configure it under the llm-pi-ai providers in Settings - Models, or reinstall this plugin to restore the default route.",
		};
		//#endregion
		...
		/**
		 * The provider route itself lives in the llm-pi-ai section (composed from
		 * this bundle's patch base under the user's settings file), so the card
		 * binds that namespace read-only to report the route a request would use.
		 */
