...
/**
 * dsh-llm-motomoto — the runtime half of the MotoMoto provider bundle.
 *
 * MotoMoto is a Codex-only relay. Two facts of its wire contract cannot be
 * expressed in the provider route's configuration:
 *
 * 1. **The protocol.** The relay serves the OpenAI *Responses* API
 *    (`/v1/responses`) and leaves `/v1/chat/completions` unwired — that path
 *    hangs until the client gives up. The route therefore declares
 *    `api: openai-responses`, which is configuration, not code.
 *
 * 2. **The client identity.** The relay admits a request by its `User-Agent`:
 *    a Codex CLI value streams, anything else is answered with
 *    `bad_response_status_code` / `openai_error`. DSH's pi-ai adapter strips
 *    any configured header colliding with its own attribution before appending
 ...
 *
 * The fence is deliberately narrow: it rewrites `user-agent` and `originator`
 * for requests addressed to the relay host, and nothing else. Every other
 * request goes to the previous `fetch` untouched. It is installed through
 * `ctx.effect()`, so stopping or reloading the plugin restores the `fetch` it
 * replaced.
 *
 ...
 * actually serves, so without this registration the browser half's status card
 * would never render.
 *
...
const Config = z.object({
  ...
  /**
   * The `originator` header Codex CLI sends alongside its User-Agent. Empty
   ...
   * the official Codex client never sends it, and the backend answers a request
   * carrying it with `400 Unsupported parameter`. The fence deletes these
   * fields from JSON request bodies addressed to the relay; the next upstream
   * complaint is a settings edit away, not a release.
   */
  stripBodyParams: z
    .array(z.string())
 ...
 * client identity: the `user-agent` the relay admits, the `originator` header
 * Codex CLI sends with it, and a request body stripped of the fields the
 ...
 * headers, and rewrites its JSON string body only when a configured field was
 * actually deleted. A bare `Request` is re-created with `duplex: 'half'` so a
 * streamed body survives the clone; its body is read through a clone, never
 * the original.
 *
...
function closeTerminalResponsesStream(response) {
  ...
  // The relay streams the whole response in a few large chunks and then holds
  // the connection open forever. A pull-based wrapper reads a fresh upstream
  // chunk on every pull, which strands whole frames in its own buffer once
  // the burst has landed (verified live: two frames delivered, then silence).
  // A TransformStream instead receives each upstream chunk exactly once and
  // drains every complete frame from it; terminate() ends the readable side
  // right after the terminal frame is forwarded, and the errored writable
  // side makes the pipe cancel the upstream connection.
  ...
  const body = response.body.pipeThrough(new TransformStream({
    transform(chunk, controller) {
      ...
      while (true) {
        ...
        if (
          /(?:^|\n)event:\s*response\.(?:completed|done|failed|incomplete)\s*(?:\n|$)/.test(frame) ||
          /"type"\s*:\s*"response\.(?:completed|done|failed|incomplete)"/.test(frame)
        ) {
          terminal = true
    ...
    flush(controller) {
      ...
      // so ordinary pass-through behavior is preserved.
      if (terminal) return
...
function fenceFetch(native, current) {
  return async function motomotoFetch(input, init) {
    ...
    if (config.originator !== '') headers.set('originator', config.originator)
    else headers.delete('originator')

    // Same URL, so the caller's own URL object or Request identity can stay:
    // only the headers and — when a strip applied — the body change. The relay
    // sometimes leaves its SSE connection open after a terminal Responses
    // event, so the returned stream is closed locally at that event.
    if (!isRequest) {
